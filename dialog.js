function showAgeVerification() {
	...
	const language = upgates.language;
	let modalTitle = "Ověření věku";
	let modalBodyText = "Bylo vám již 18 let?";
	let yesButtonText = "Ano";
	let noButtonText = "Ne";
	if (language === 'en' || language === 'tl') {
		modalTitle = "Age Verification";
		modalBodyText = "Are you at least 18 years old?";
		yesButtonText = "Yes";
		noButtonText = "No";
	}
	const modalHTML = `
	...
	document.getElementById('ageVerificationYes').onclick = function () {
		localStorage.setItem('ageVerified', !0);
...
const breadcrumbList = document.querySelector('ol[itemtype="http://schema.org/BreadcrumbList"]');
if (breadcrumbList) {
	const listItems = breadcrumbList.querySelectorAll('li');
	for (const item of listItems) {
		if (item.textContent.includes("Alcoholic") || item.textContent.includes("Alkohol")) {
			showAgeVerification();
			break;
		}
	}
